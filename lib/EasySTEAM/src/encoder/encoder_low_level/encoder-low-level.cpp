...
void EncoderLL::initialize(gpio_num_t pinA, gpio_num_t pinB)
{
    ...
    if(direction_result)
        ESP_LOGE(TAG, "Failed to set pins[A, B](%d, %d) to INPUT MODE", aPinNumber, bPinNumber);

    esp_err_t pulldown_result = gpio_pulldown_en(aPinNumber);
    pulldown_result |= gpio_pulldown_en(bPinNumber);

    if(pulldown_result)
        ESP_LOGE(TAG, "Failed to set pins[A, B](%d, %d) to PULLDOWN MODE", aPinNumber, bPinNumber);
    ...
    if(unit_result)
        ESP_LOGE(TAG, "Failed to set pins[A, B](%d, %d) on Encoder with code: %s", aPinNumber, bPinNumber, esp_err_to_name(unit_result));
    ...
    if(channelA_result)
        ESP_LOGE(TAG, "Failed to create channel A with code: %s", esp_err_to_name(channelA_result));
    ...
    if(channelB_result)
        ESP_LOGE(TAG, "Failed to create channel B with code: %s", esp_err_to_name(channelB_result));
    ...
    if(channelA_cfg_result)
        ESP_LOGE(TAG, "Failed to configure channel A");
    ...
    if(channelB_cfg_result)
        ESP_LOGE(TAG, "Failed to configure channel B");
...
void EncoderLL::setFilterInNanoseconds(uint32_t nanoseconds)
{
    ...
    if(filter_result)
        ESP_LOGE(TAG, "Failed to apply filter, code: %s\n", esp_err_to_name(filter_result));
}
...
void EncoderLL::reset()
{
    ...
    if(reset_result)
        ESP_LOGE(TAG, "Failed to clear counts of encoder with code: %s\n", esp_err_to_name(reset_result));
}
...
void EncoderLL::stop()
{
    ...
    if(stop_result)
        ESP_LOGE(TAG, "Failed to stop encoder with code: %s\n", esp_err_to_name(stop_result));
}
...
void EncoderLL::enable()
{
    ...
    if(enable_result)
        ESP_LOGE(TAG, "Failed to enable encoder with code: %s\n", esp_err_to_name(enable_result));
}
...
void EncoderLL::disable()
{
    ...
    if(disable_result)
        ESP_LOGE(TAG, "Failed to disable encoder with code: %s\n", esp_err_to_name(disable_result));
}
...
void EncoderLL::start()
{
    ...
    if(start_result)
        ESP_LOGE(TAG, "Failed to start encoder with code: %s\n", esp_err_to_name(start_result));
}