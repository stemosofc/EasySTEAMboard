...
void Servo::write(int value)
{
    ...
    {
        if (value < MIN_DEGREES)
            value = MIN_DEGREES;
        else if (value > MAX_DEGREES)
            value = MAX_DEGREES;

        value = map(value, MIN_DEGREES, MAX_DEGREES, this->min, this->max);
    }
...
{
    return (map(readMicroseconds(), this->min, this->max, MIN_DEGREES, MAX_DEGREES));
}
